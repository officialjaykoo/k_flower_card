...
from .desgenome import DesGenome
from .environment import EnvironmentDescription
from .network import Graph
from .search import Search, SearchConnection, SearchResult
...
def topology_init_config(
    topology_or_description,
    control_snapshot: ControlSnapshot | None = None,
) -> dict[str, Any]:
    if isinstance(topology_or_description, EnvironmentDescription):
        description = topology_or_description
        metadata = dict(description.metadata or {})
        topology = metadata.get("topology")
        snapshot = metadata.get("control_snapshot")
        if topology is None or snapshot is None:
            raise RuntimeError("EnvironmentDescription metadata must include topology and control_snapshot")
        control_snapshot = snapshot
    else:
        topology = topology_or_description
    if control_snapshot is None:
        raise RuntimeError("control_snapshot is required")
    return {
        "seed_graph": TopologyGraph.from_substrate_topology(topology),
        "control_snapshot": copy.deepcopy(control_snapshot),
    }
...
class Developer:
    ...
    def __post_init__(self) -> None:
        self.search_engine = Search(self.config.search)

    def develop(self, genome: DesGenome) -> Graph:
    ...
    def _is_substrate_enabled(self, genome: DesGenome, substrate: SubstrateRef) -> bool:
    ...
    def _is_link_enabled(self, genome: DesGenome, edge: DevelopmentEdge) -> bool:
    ...
    def _depth_for(self, spec: SubstrateSpec, genome: DesGenome) -> int:
    ...
    def _genome_debug_key(self, genome: DesGenome) -> int | None:
        ...
        try:
            ...
            return None
__all__ = ["DesGenome", "Developer", "topology_init_config"]
