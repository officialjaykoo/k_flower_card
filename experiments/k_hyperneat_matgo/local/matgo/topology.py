...
TYPE_FEATURES = frozenset({"kwang", "five", "ribbon", "junk", "pi", "bonus"})
...
def _group_specs(cfg: MatgoLayoutConfig) -> list[SubstrateSpec]:
    ...
    return [
        ...
        *_zone_group_specs("board", month_y=-0.24, type_y=-0.28, combo_y=-0.32),
        ...
        *_zone_group_specs("captured_opp", month_y=-0.34, type_y=-0.38, combo_y=-0.42),
        ...
        SubstrateSpec(
            SubstrateRef("hidden_group_public_context", 0),
            seed_points=[],
            depth=0,
        ),
    ]
...
def _group_targets_for_input(input_kind: str) -> list[SubstrateRef]:
    ...
    if kind == "input_public":
        return [SubstrateRef("hidden_group_public_context", 0)]
    for zone_name in ("hand", "board", "captured_self", "captured_opp"):
...
def _hidden_targets_for_group(group_kind: str) -> list[SubstrateRef]:
    ...
    if kind == "hidden_group_hand_combo":
        return [play_hidden, option_hidden]
    if kind == "hidden_group_board_month" or kind == "hidden_group_board_type":
        return [play_hidden, match_hidden]
    if kind == "hidden_group_board_combo":
    ...
    if kind == "hidden_group_ownership_context":
        return [play_hidden, match_hidden, option_hidden]
    if kind == "hidden_group_public_context":
        return [play_hidden, match_hidden, option_hidden]
    return [play_hidden, match_hidden, option_hidden]
...
def _input_to_group_weight(source_kind: str, target_kind: str) -> float:
    ...
    if source_kind == "input_rule_score" and target_kind == "hidden_group_rule_context":
        return 1.25
    if source_kind == "input_public" and target_kind == "hidden_group_public_context":
        return 1.10
    if source_kind.startswith("input_hand_"):
        return 1.05
...
def _group_to_hidden_weight(source_kind: str, target_kind: str) -> float:
    if source_kind == "hidden_group_rule_context":
        ...
        if target_kind == "hidden_play_context":
            return 0.85
        return 0.75
    if source_kind == "hidden_group_public_context":
        if target_kind == "hidden_play_context":
            return 1.05
        if target_kind == "hidden_option_context":
            return 0.90
...
def build_minimal_matgo_topology(config: MatgoLayoutConfig | None = None) -> SubstrateTopology:
    ...
    inputs = [
        SubstrateSpec(public_ref, seed_points=_points_from_xs(PUBLIC_XS, 1.0)),
    ]
    inputs.extend(_rule_score_specs(y=0.88, x_start=-0.95, x_end=0.95))
    inputs.extend(_feature_specs("hand", cfg.hand_slots, y_top=0.76, y_bottom=0.56))
    inputs.extend(_semantic_bin_specs("board", month_y=0.46, type_y=0.40, combo_y=0.34))
    ...
    outputs = [
        SubstrateSpec(play_ref, seed_points=_row(cfg.hand_slots, y=-0.84)),
        SubstrateSpec(match_ref, seed_points=_points_from_xs(MONTH_XS, -0.92)),
