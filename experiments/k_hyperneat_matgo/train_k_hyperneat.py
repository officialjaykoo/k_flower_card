        ...
        for genome_key, genome in (population or {}).items():
            ...
            if is_first_seen:
                ...
                if len(parents) >= 2:
                    ...
                    self.origin_by_key[key] = "mutation"
                else:
...
class KHyperneatTrainer:
    def __init__(self, runtime: dict[str, Any], config: Any, out_dir: Path):
        ...
        self.runtime_model_cache: dict[int, dict[str, Any]] = {}
    ...
    def build_runtime_model(self, genome_id: int, genome: Any, config: Any) -> dict[str, Any]:
        ...
        with self.runtime_model_cache_lock:
            cached = self.runtime_model_cache.get(cache_key)
        if cached is not None:
            return cached

        developer = Developer(self.des_config)
        developed = developer.develop(genome)
        ...
        with self.runtime_model_cache_lock:
            cached = self.runtime_model_cache.get(cache_key)
            if cached is None:
                self.runtime_model_cache[cache_key] = runtime_model
                cached = runtime_model
        return cached
    ...
    def run_duel_eval(self, runtime_path: Path, result_path: Path, requested_games: int | None = None, use_early_stop: bool = True) -> dict[str, Any]:
        ...
        checkpoints = [int(item["games"]) for item in cutoffs] + [total_games]
...
def main() -> None:
    ...
    checkpointer_class = Checkpointer if args.stdout_format == "text" else QuietCheckpointer
