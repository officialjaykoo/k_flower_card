...
def build_chunk_sizes(total_games, chunk_games):
    sizes = []
    remaining = int(total_games)
    while remaining > 0:
        g = min(int(chunk_games), remaining)
        if g % 2 != 0:
            g -= 1
        if g <= 0:
            g = 2
...
def build_fixed_opponent_plan(opponents, num_chunks, rng):
    if num_chunks <= 0:
        return []
    ws = [max(0.0, float(i.get("weight", 1.0))) for i in opponents]
    total = sum(ws)
    if total <= 0:
        ws = [1.0 for _ in opponents]
        total = float(len(opponents))
    targets = [w * float(num_chunks) / total for w in ws]
    counts = [int(t) for t in targets]
    remain = int(num_chunks - sum(counts))
    fracs = sorted(
        [(targets[i] - counts[i], i) for i in range(len(opponents))],
...
def main():
    ...
    parser.add_argument(
        "--pool-mode",
        choices=["fixed", "weighted_random"],
        default="fixed",
        help="Opponent sampling mode: fixed (default, weighted exact schedule) or weighted_random.",
    )
    ...
    chunk_sizes = build_chunk_sizes(args.total_games, chunk_games)
    remaining = args.total_games
    side_flip = False
    chunk_infos = []
    aggregate = {"games": 0, "completed": 0, "winners": {"mySide": 0, "yourSide": 0, "draw": 0, "unknown": 0}, "matchups": {}}
    opponent_plan = (
        build_fixed_opponent_plan(opponents, len(chunk_sizes), rng)
        if args.pool_mode == "fixed"
        else []
    )

    for i, g in enumerate(chunk_sizes, start=1):
        if args.pool_mode == "fixed":
            opp = opponents[opponent_plan[i - 1]]
        else:
