...
def normalize_runtime(cfg: Dict[str, Any], cfg_path: str) -> Dict[str, Any]:
    ...
    # Optional entropy annealing endpoint.
...
def train(
    ...
    try:
        ...
        for update in range(resume_update + 1, runtime["total_updates"] + 1):
            ...
            if runtime["entropy_coef_final"] is None:
                ...
                if runtime["total_updates"] <= 1:
                    ...
                    entropy_coef_now = float(
                        runtime["entropy_coef"]
                        + progress * (runtime["entropy_coef_final"] - runtime["entropy_coef"])
                    )
                if entropy_coef_now < 0.0:
                    entropy_coef_now = 0.0
            ...
            for _ in range(runtime["rollout_steps"]):
                ...
                forced_go_flags = [False] * num_envs
                ...
                with torch.no_grad():
                    ...
                    if any(forced_go_flags):
                        forced_mask = torch.tensor(forced_go_flags, dtype=torch.bool, device=device)
                        go_actions = torch.full((num_envs,), GO_ACTION_INDEX, dtype=torch.int64, device=device)
                        go_logprob = dist.log_prob(go_actions)
                        logprob = torch.where(forced_mask, go_logprob, logprob)

                policy_weights = torch.tensor(
                    [
                        runtime["forced_go_policy_weight"] if forced else 1.0
                        for forced in forced_go_flags
                    ],
                    dtype=torch.float32,
                    device=device,
                )
