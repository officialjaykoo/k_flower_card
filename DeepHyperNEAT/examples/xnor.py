...
def xnor(genomes):
	...
	expected_outputs = [1.0, 0.0, 0.0, 1.0]
	...
	for genome_key, genome in genomes:
		...
		for inputs, expected in zip(xnor_inputs, expected_outputs):
			inputs = inputs + (1.0,)
...
winner_genome = pop.run(xnor,goal_fitness,num_generations)
...
draw_net(cppn, filename="reports/champion_images/xnor_cppn")
draw_net(substrate, filename="reports/champion_images/xnor_substrate")

# Run winning genome on the task again
print("\nChampion Genome: {} with Fitness {}\n".format(winner_genome.key,
	  											winner_genome.fitness))
xnor([(winner_genome.key,winner_genome)])
