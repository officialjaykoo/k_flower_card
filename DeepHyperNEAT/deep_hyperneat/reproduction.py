...
class Reproduction:
	...
	def __init__(self, species_elitism=1, species_reproduction_threshold=0.2):
		self.genome_indexer = count(1)
		self.reporters = None
		# Number of elites allowed to be cloned into species each gen
		self.species_elitism = max(0, int(species_elitism))
		self.stagnation = Stagnation(self.species_elitism)
		# Fraction of members of a species allowed to reproduce each gen
		self.species_reproduction_threshold = max(0.0, min(1.0, float(species_reproduction_threshold)))
		self.ancestors = {}
		self.birth_generation = {}
		self.origin_by_key = {}

	def create_new_population(self, num_genomes, generation=0):
		'''
		...
		for i in range(num_genomes):
			...
			self.ancestors[int(gid)] = tuple()
			self.birth_generation[int(gid)] = int(generation)
			self.origin_by_key[int(gid)] = "init"
		...
		for spawn, species in zip(spawn_amounts, remaining_species):
			...
			if self.species_elitism > 0:
				for member_key, member in old_species_members[:self.species_elitism]:
					...
					new_ancestors[int(member_key)] = tuple(self.ancestors.get(int(member_key), tuple()))
					new_birth_generation[int(member_key)] = int(
						self.birth_generation.get(int(member_key), generation)
					)
					new_origin_by_key[int(member_key)] = str(
						self.origin_by_key.get(int(member_key), "init") or "init"
					)
					spawn -= 1
			...
			while spawn > 0:
				...
				new_ancestors[int(child_key)] = (int(parent1_key),)
				new_birth_generation[int(child_key)] = int(generation) + 1
				new_origin_by_key[int(child_key)] = "offspring"
		self.ancestors = new_ancestors
		self.birth_generation = new_birth_generation
		self.origin_by_key = new_origin_by_key
