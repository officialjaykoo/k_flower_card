...
class Population():
	...
	def __init__(self, key, size, elitism=1, state=None,
				 species_threshold=3.5,
				 species_reproduction_threshold=0.2):
		'''
		...
		self.reproduction = Reproduction(species_elitism=self.elitism,
										 species_reproduction_threshold=species_reproduction_threshold)
		self.species = SpeciesSet(species_threshold)
	...
	def run(self,task,goal,generations=None,generation_callback=None,reset_generation=True,emit_builtin_reports=True):
		'''
		...
		while self.current_gen < generations and not reached_goal:
			...
			if not reached_goal:
				...
				if not self.species.species:
					...
					self.population = self.reproduction.create_new_population(self.size, generation=self.current_gen + 1)

				# Speciate new population
				self.species.speciate(self.population, self.current_gen)
			self.current_gen += 1
			if generation_callback is not None:
				generation_callback(self, reached_goal)
